...
class minimum_snap:
    ...
    def p_constrain(self, vx0=0, ax0=0, vxk=0, axk=0, vy0=0, ay0=0, vyk=0, ayk=0):
        ...
        # end node
        self.p_y[k + 2][0] = self.list_y[k]
        self.p_y[k + 3][0] = vyk
        self.p_y[k + 4][0] = ayk

        print('p is done')
        print('k+5的值:', k + 5)
        print('px的总长度:', len(self.p_x), 'py的总长度:', len(self.p_y), '4k+2的值:', 4 * k + 2)
    ...
    def M_constrain(self):
        ...
        for i in range(n + 1):
            ...
            if i == n - 1:
                continue
            M1[k + 4][i + 2 + (k - 1) * (n + 1)] = (i + 2) * (i + 1) * self.t_to[-1] ** i
        ...
        while l < 3 * (k - 1):
            for i in range(n + 1):
                ...
                if i == n:
                    continue
                M2[l + 1][j * (n + 1) + i + 1] = (i + 1) * self.t_to[j + 1] ** i
                M2[l + 1][j * (n + 1) + i + 1 + (n + 1)] = -(i + 1) * self.t_to[j + 1] ** i
                if i == n - 1:
                    continue
                M2[l + 2][j * (n + 1) + i + 2] = (i + 2) * (i + 1) * self.t_to[j + 1] ** i
                M2[l + 2][j * (n + 1) + i + 2 + (n + 1)] = - (i + 2) * (i + 1) * self.t_to[j + 1] ** i
    ...
    def figure_out(self):
        ...
        for t_a in range(len(self.t_to) - 1):
            time_list = np.linspace(self.t_to[t_a], self.t_to[t_a + 1], Dis_num, endpoint=True)
            for j in time_list:
